...
import { Edit3, Trash2, Copy, Plus, Search, Droplets, Layers, Timer, Calendar, Clock, FileSpreadsheet, SlidersHorizontal, X } from "lucide-react";
...
const ProgramsList = () => {
  ...
  return (
    <AppShell>
      <div className="relative mb-5 overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-primary/10 via-background to-acid/10 p-4 sm:p-5 shadow-soft">
        <div className="absolute -top-10 -right-10 size-32 rounded-full bg-primary/15 blur-2xl pointer-events-none" />
        <div className="absolute -bottom-12 -left-8 size-28 rounded-full bg-acid/15 blur-2xl pointer-events-none" />
        <div className="relative flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.18em] font-bold text-primary/80 mb-1">
              <Droplets className="size-3" /> Irrigazione
            </div>
            <h1 className="text-2xl sm:text-3xl font-black leading-tight tracking-tight bg-gradient-to-br from-foreground to-foreground/70 bg-clip-text text-transparent">
              I miei programmi
            </h1>
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-background/70 backdrop-blur border border-border/60 text-xs font-semibold">
                <span className="tabular-nums">{programs.length}</span>
                <span className="text-muted-foreground font-medium">totali</span>
              </span>
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/15 border border-primary/20 text-xs font-semibold text-primary">
                <span className="size-1.5 rounded-full bg-primary animate-pulse" />
                <span className="tabular-nums">{programs.filter(p => p.active).length}</span>
                <span className="opacity-80 font-medium">attivi</span>
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              <FileSpreadsheet className="size-4" /> <span className="hidden sm:inline">Esporta</span>
            </Button>
            <Popover>
              <PopoverTrigger asChild>
                <Button className="shadow-glow"><Plus className="size-4" /> Nuovo</Button>
              </PopoverTrigger>
